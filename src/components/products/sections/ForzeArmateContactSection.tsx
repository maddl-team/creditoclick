  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/contact", {
        ...
        body: JSON.stringify({
          formType: "Forze Armate e dell'Ordine",
          subject: "Nuova richiesta - Forze Armate e dell'Ordine",
          ...
          data: {
            ...
            anniServizio,
            stipendioNetto: `${stipendioNetto} EUR`,
            orarioContatto,
            ...
            <form onSubmit={step === 4 ? onSubmit : (e) => e.preventDefault()} className="space-y-5">
              {step === 1 && (
                <>
                  ...
                  <label htmlFor="fam-corpo" className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Corpo</span><select id="fam-corpo" value={corpo} onChange={(e) => setCorpo(e.target.value as Corpo)} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3"><option value="carabinieri">Carabinieri</option><option value="polizia">Polizia</option><option value="gdf">GDF</option><option value="esercito">Esercito</option><option value="marina">Marina</option><option value="aeronautica">Aeronautica</option><option value="penitenziaria">Penitenziaria</option></select></label>
                  <label htmlFor="fam-grado" className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Grado (opzionale)</span><input id="fam-grado" type="text" value={grado} onChange={(e) => setGrado(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3" /></label>
                </>
              )}
              {step === 2 && (
                <>
                  <h3 className="text-2xl font-bold text-text-primary">Step 2: Servizio</h3>
                  <label htmlFor="fam-anni-servizio" className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Anni di servizio</span><input id="fam-anni-servizio" type="number" min={0} max={50} value={anniServizio} onChange={(e) => setAnniServizio(Math.trunc(Number(e.target.value) || 0))} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3" />{errors.anniServizio && <p className="text-xs text-red-600">{errors.anniServizio}</p>}</label>
                  <label htmlFor="fam-stipendio" className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Stipendio netto (indennita incluse)</span><input id="fam-stipendio" type="number" min={600} value={stipendioNetto} onChange={(e) => setStipendioNetto(Math.trunc(Number(e.target.value) || 0))} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3" />{errors.stipendio && <p className="text-xs text-red-600">{errors.stipendio}</p>}</label>
                </>
              )}
              {step === 3 && (
                <>
                  <h3 className="text-2xl font-bold text-text-primary">Step 3: Preferenze</h3>
                  <label htmlFor="fam-orario-contatto" className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Orario per contatto</span><select id="fam-orario-contatto" value={orarioContatto} onChange={(e) => setOrarioContatto(e.target.value as Contatto)} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3"><option value="mattina">Mattina</option><option value="pomeriggio">Pomeriggio</option><option value="sera">Sera</option></select></label>
                </>
              )}
              {step === 4 && (
                <>
                  <h3 className="text-2xl font-bold text-text-primary">Step 4: Contatto</h3>
                  <label htmlFor="fam-nome" className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Nome</span><input id="fam-nome" type="text" value={nome} onChange={(e) => setNome(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3" />{step4SubmitAttempted && errors.nome && <p className="text-xs text-red-600">{errors.nome}</p>}</label>
                  <label htmlFor="fam-cognome" className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Cognome</span><input id="fam-cognome" type="text" value={cognome} onChange={(e) => setCognome(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3" />{step4SubmitAttempted && errors.cognome && <p className="text-xs text-red-600">{errors.cognome}</p>}</label>
                  <label htmlFor="fam-cellulare" className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Cellulare</span><input id="fam-cellulare" type="tel" placeholder="+39XXXXXXXXXX" value={cellulare} onChange={(e) => setCellulare(normalizePhone(e.target.value))} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3" />{step4SubmitAttempted && errors.cellulare && <p className="text-xs text-red-600">{errors.cellulare}</p>}</label>
                  <label htmlFor="fam-email" className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Email</span><input id="fam-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3" />{step4SubmitAttempted && errors.email && <p className="text-xs text-red-600">{errors.email}</p>}</label>
