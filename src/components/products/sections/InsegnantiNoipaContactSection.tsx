  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      pushContactLeadEvent(
        {
          formSource: "Insegnanti e Scuola (NoiPA)",
          email: email.trim(),
          phone: normalizePhone(cellulare),
        },
            ...
            <form onSubmit={step === 4 ? onSubmit : (e) => e.preventDefault()} className="space-y-5">
              {step === 1 && (
                <>
                  ...
                  <label htmlFor="noipa-ruolo" className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Ruolo</span><select id="noipa-ruolo" value={ruolo} onChange={(e) => setRuolo(e.target.value as Ruolo)} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3"><option value="docente">Docente</option><option value="personale_ata">Personale ATA</option></select></label>
                  <label htmlFor="noipa-grado" className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Grado scolastico</span><select id="noipa-grado" value={gradoScolastico} onChange={(e) => setGradoScolastico(e.target.value as Grado)} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3"><option value="infanzia_primaria">Infanzia/Primaria</option><option value="secondaria">Secondaria</option><option value="universita">Universita</option></select></label>
                  <label htmlFor="noipa-stato-giuridico" className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Stato giuridico</span><select id="noipa-stato-giuridico" value={statoGiuridico} onChange={(e) => setStatoGiuridico(e.target.value as Stato)} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3"><option value="di_ruolo">Di ruolo</option><option value="supplente_annuale">Supplente annuale</option></select></label>
                </>
              )}
              {step === 2 && (
                <>
                  <h3 className="text-2xl font-bold text-text-primary">Step 2: Integrazione Digitale</h3>
                  <label htmlFor="noipa-cedolino-pdf" className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Disponibilita Cedolino PDF</span><select id="noipa-cedolino-pdf" value={cedolinoPdf} onChange={(e) => setCedolinoPdf(e.target.value as Cedolino)} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3"><option value="si">Si</option><option value="no">No</option></select></label>
                  <label htmlFor="noipa-stipendio" className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Stipendio mensile</span><input id="noipa-stipendio" type="number" min={600} value={stipendioMensile} onChange={(e) => setStipendioMensile(Math.trunc(Number(e.target.value) || 0))} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3" />{errors.stipendio && <p className="text-xs text-red-600">{errors.stipendio}</p>}</label>
                </>
              )}
              {step === 3 && (
                <>
                  <h3 className="text-2xl font-bold text-text-primary">Step 3: Situazione Debitoria</h3>
                  <label htmlFor="noipa-cessioni" className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Cessioni attive</span><select id="noipa-cessioni" value={cessioniAttive} onChange={(e) => setCessioniAttive(e.target.value as Cessioni)} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3"><option value="si">Si</option><option value="no">No</option><option value="non_ricordo">Non ricordo</option></select></label>
                </>
              )}
              {step === 4 && (
                <>
                  <h3 className="text-2xl font-bold text-text-primary">Step 4: Contatto</h3>
                  <label htmlFor="noipa-nome" className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Nome</span><input id="noipa-nome" type="text" value={nome} onChange={(e) => setNome(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3" />{step4SubmitAttempted && errors.nome && <p className="text-xs text-red-600">{errors.nome}</p>}</label>
                  <label htmlFor="noipa-cognome" className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Cognome</span><input id="noipa-cognome" type="text" value={cognome} onChange={(e) => setCognome(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3" />{step4SubmitAttempted && errors.cognome && <p className="text-xs text-red-600">{errors.cognome}</p>}</label>
                  <label htmlFor="noipa-cellulare" className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Cellulare</span><input id="noipa-cellulare" type="tel" placeholder="+39XXXXXXXXXX" value={cellulare} onChange={(e) => setCellulare(normalizePhone(e.target.value))} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3" />{step4SubmitAttempted && errors.cellulare && <p className="text-xs text-red-600">{errors.cellulare}</p>}</label>
                  <label htmlFor="noipa-email" className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Email</span><input id="noipa-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3" />{step4SubmitAttempted && errors.email && <p className="text-xs text-red-600">{errors.email}</p>}</label>
              ...
              <div className="flex flex-col gap-3 pt-2">
                {step < 4 ? <Button key="continue-button" type="button" className="w-full bg-brand-indigo text-white hover:bg-brand-indigo/90" icon={ArrowRight} onClick={goNext}>Continua</Button> : <Button key="submit-button" type="submit" disabled={isSubmitting} className="w-full bg-brand-indigo text-white hover:bg-brand-indigo/90" icon={ArrowRight}>{isSubmitting ? "Invio in corso..." : "Invia"}</Button>}
