...
export function PensionatiInpsContactSection() {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    if (Object.keys(next).length) return;
    const text = [
      "Nuova richiesta - Pensionati INPS",
      "",
      "STEP 1: Verifica Eta",
      `Data di nascita: ${dataNascita}`,
      "",
      "STEP 2: Dettagli Pensione",
      `Importo netto: ${importoNetto} EUR`,
      `Tipologia: ${tipologiaPensione}`,
      "",
      "STEP 3: Documentazione",
      `Comunicazione di cedibilita in possesso: ${comunicazioneCedibilita}`,
      "",
      "STEP 4: Contatto",
      `Nome: ${nome}`,
      `Cognome: ${cognome}`,
      `Cellulare: ${normalizePhone(cellulare)}`,
      `Telefono fisso: ${telefonoFisso ? normalizePhone(telefonoFisso) : "Non indicato"}`,
      `Email: ${email}`,
            ...
            <form onSubmit={step === 4 ? onSubmit : (e) => e.preventDefault()} className="space-y-5">
              ...
              {step === 4 && (
                <>
                  ...
                  <label className="block space-y-2"><span className="text-sm font-semibold text-text-primary">Telefono fisso</span><input type="tel" placeholder="+390836311982" value={telefonoFisso} onChange={(e) => setTelefonoFisso(normalizePhone(e.target.value))} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3" />{errors.fisso && <p className="text-xs text-red-600">{errors.fisso}</p>}</label>
              ...
              <div className="flex flex-col gap-3 pt-2">
                {step < 4 ? <Button type="button" className="w-full bg-brand-indigo text-white hover:bg-brand-indigo/90" icon={ArrowRight} onClick={nextStep}>Continua</Button> : <Button type="submit" className="w-full bg-brand-indigo text-white hover:bg-brand-indigo/90" icon={ArrowRight}>Invia</Button>}
